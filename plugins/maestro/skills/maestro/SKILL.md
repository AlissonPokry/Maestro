...
If `python` is unavailable, use any available Python runtime. If no path is saved, ask for the path and run `/maestro-set <skill-bundle-folder>`. If the script cannot run, manually inspect immediate child bundle folders and update the `Known Bundles` section below.
